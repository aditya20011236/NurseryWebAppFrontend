...
function PL_Q() {
  ...
  const fetchData = async () => {
    try {
      ...
      const startDate = `${selectedYear}-${quarterStartMonth
        .toString()
        .padStart(2, "0")}-${startDay}`;
      const endDate = `${selectedYear}-${quarterEndMonth
        .toString()
        .padStart(2, "0")}-${endDay}`;
      ...
      const totalExp = expensesResponse.data.reduce(
        (acc, item) => acc + Number(item.grandTotal),
        0
      );
      setTotalExpenses(totalExp);

      const totalSalesQuarter = salesResponse.data.reduce(
        (acc, item) => acc + Number(item.grandtotal),
        0
      );
    ...
    <div className="flex flex-col h-screen">
      ...
      <div className="max-w-xl mx-auto p-4 bg-white shadow-md ml-0">
        ...
        {expenseData.length > 0 && (
          <p className="text-sm">
            Start Date of Quarter {selectedQuarter} is "{expenseData[0].date}"
            and End Date of Quarter {selectedQuarter} is "
            {expenseData[expenseData.length - 1].date}"
          </p>
        )}
      </div>
