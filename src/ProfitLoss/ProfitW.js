...
function PL_week() {
  ...
  useEffect(() => {
    if (selectedYear && selectedMonth) {
      ...
      }-${selectedMonthObj.days.toString().padStart(2, "0")}`;
  ...
  useEffect(() => {
    if (selectedWeek) {
      ...
      const startDate = weeksInMonth[selectedWeekIndex].startDate
        .toISOString()
        .split("T")[0];
      const endDate = weeksInMonth[selectedWeekIndex].endDate
        .toISOString()
        .split("T")[0];
      fetchData(startDate, endDate);
    }
  ...
  const fetchData = async (startDate, endDate) => {
    try {
      ...
      const totalExp = expenseResponse.data.reduce(
