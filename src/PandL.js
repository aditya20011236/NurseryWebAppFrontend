...
//         setResult(`Your date is ${formatDate(selectedDate)}, it is the ${getDayOfYear(selectedDate)}th day of financial year ${currentYear}.`);
//         break;
//       case 'Weeks':
//         setStartDate(getFirstDayOfWeek(selectedDate));
//         setResult(`Your date is ${formatDate(selectedDate)}, it is the ${getWeekOfYear(selectedDate)}th week of financial year ${currentYear} and the first day of the week is ${getFirstDayOfWeek(selectedDate)}.`);
//         break;
//       case 'Months':
//         setStartDate(getFirstDayOfMonth(selectedDate));
//         setResult(`Your date is ${formatDate(selectedDate)}, it is the ${getMonthOfYear(selectedDate)}th month of financial year ${currentYear} and the first day of the month is ${getFirstDayOfMonth(selectedDate)}.`);
//         break;
//       case 'Quarters':
//         setStartDate(getFirstDayOfQuarter(selectedDate));
//         setResult(`Your date is ${formatDate(selectedDate)}, it is the ${getQuarterOfYear(selectedDate)}th quarter of financial year ${currentYear} and the first day of the quarter is ${getFirstDayOfQuarter(selectedDate)}.`);
//         break;
//       case 'Years':
//         setStartDate(getFirstDayOfYear(selectedDate));
//         setResult(`Your date is ${formatDate(selectedDate)}, it is financial year ${currentYear} and the first day of the year is ${getFirstDayOfYear(selectedDate)}.`);
//         break;
//       default:
//         setResult('');
//     }
...
//     const firstDayOfQuarter = new Date(date.getFullYear(), (quarter - 1) * 3, 1);
...
//           <button onClick={calculateResult} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-r-md focus:outline-none">Calculate</button>
//         </div>
//         <select value={calculationType} onChange={handleCalculationTypeChange} className="border border-gray-300 rounded-md px-3 py-2 w-full focus:outline-none">
...
//                 <tr key={index} className="bg-gray-100 text-center divide-y divide-x divide-solid divide-gray-500">
//                   <td className="py-2 px-4 divide-y divide-solid divide-gray-500">{item.id}</td>
...
//           </tbody>
//         </table>
//         <h1 className='text-5xl font-bold'> Total Sales = {totalSales} and Total Expance ={totalExpenses}  </h1>
//       </div>
//     </div>
//   );
// }
...
function PandL() {
  ...
  const fetchData = async () => {
    try {
      ...
      const salesTotal = salesResponse.data.reduce((acc, item) => acc + Number(item.grandtotal), 0);
    ...
    try {
      ...
      const expenseTotal = expenseResponse.data.reduce((acc, item) => acc + Number(item.grandTotal), 0);
        ...
        <table className='w-full border-collapse border border-gray-400'>
          ...
          <tbody>
            ...
            {salesData.map((saleItem, index) => (
              <tr key={index} className="bg-gray-100 text-center divide-y divide-x divide-solid divide-gray-500">
                <td className="py-2 px-4 divide-y divide-solid divide-gray-500">{saleItem.id}</td>
                ...
                <td className="py-2 px-4">{expenseData[index]?.grandTotal || 0}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {/* Display total sales, total expenses, and profit or loss */}
        <h1 className='text-5xl font-bold'> Total Sales = {totalSales}, Total Expenses = {totalExpenses}, Profit/Loss = {profitOrLoss} </h1>
      </div>
    </div>
  );
}
