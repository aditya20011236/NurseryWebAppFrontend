...
import EmpMenu from "./empsidebar";
...
function EmpInvoice() {
  ...
  const handleProductSelect = (index, productId) => {
    ...
    if (selectedProduct) {
      ...
      products[index]["availableQuantity"] = selectedProduct.availableQuantity; // Add available quantity
  ...
  const handleInputChange = (index, event) => {
    ...
    if (name === "quantity") {
      const availableQuantity = products[index]["availableQuantity"]; // Get available quantity
  ...
  const calculateTotal = () => {
    ...
    if (
      ...
      const discountAmount = (grandTotal * (formData.discount / 100)).toFixed(
        2
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(host + "/api/invoices", formData);
  ...
  const generatePDF = (grandTotal, billNo, customerName) => {
      ...
      didDrawCell: function (data) {
        if (data.row.index === formData.products.length) {
          if (data.column.index === 0) {
            pdf.setFont("bold");
            pdf.text("", data.cell.x + 2, data.cell.y + data.cell.height / 2, {
              align: "left",
              baseline: "middle",
            });
            ...
            const startX =
              data.cell.x + cellWidth / 1.3 - grandTotalTextWidth / 2;
            pdf.setFont("bold");
            pdf.text(
    ...
    const additionalDetailsWidth = 181;
    ...
    pdf.autoTable({
      ...
      columnStyles: {
        0: { halign: "left", fontStyle: "bold", cellWidth: "wrap" },
      },
      margin: { bottom: 30 },
      ...
      <div className="container mx-auto mt-5 bg-white p-5 rounded-lg shadow-lg">
        ...
        >
          ...
          <div>
            ...
            <input
              type="date"
              className="appearance-none border rounded w-32 py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              id="date"
              name="date"
              value={formData.date}
              onChange={(e) =>
