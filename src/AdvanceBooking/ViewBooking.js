                                ...
                                <tbody>
                                    {customers.map((customer, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="border px-4 py-2">{customer.customerAddress}</td>
                                            <td className="border px-4 py-2">{customer.customerPhoneNumber}</td>
                                            <td className="border px-4 py-2">{customer.plants.map(plant => plant.plantName).join(', ')}</td>
                                            <td className="border px-4 py-2">{customer.plants.map(plant => formatCurrency(plant.price)).join(', ')}</td>
                                            <td className="border px-4 py-2">{customer.plants.map(plant => plant.quantity).join(', ')}</td>
                                            <td className="border px-4 py-2">{formatDate(customer.deliveryDate)}</td>
                                            <td className="border px-4 py-2">{formatCurrency(customer.grandTotal)}</td>
                                            <td className="border px-4 py-2">{formatCurrency(customer.advancePayment)}</td>
                                            <td className="border px-4 py-2">{formatCurrency(customer.remainingPayment)}</td>
                                            <td className="border px-4 py-2">
                                                <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded" onClick={() => handleDelete(customer.id)}>Delivered</button>
                                                <div className="text-center mt-1">
                                                    <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded" onClick={generateBill}>Generate Bill</button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
