...
function AdvanceBooking() {
  ...
  const fetchLatestId = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/customers/latestId');
  ...
  const handleInputChange = (index, event) => {
    ...
    if (name === 'quantity') {
      ...
      if (selectedProduct && parseInt(value) > selectedProduct.availableQuantity) {
        alert(`Quantity cannot be above available quantity (${selectedProduct.availableQuantity})`);
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, customerPhoneNumber: e.target.value.replace(/\D/, '') })}
              />
            </div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (enteredValue > grandTotal) {
                          alert("Advance payment cannot be greater than the grand total.");
                        } else {
                          setFormData({ ...formData, advancePayment: enteredValue });
                        }
                      }}
                    />
